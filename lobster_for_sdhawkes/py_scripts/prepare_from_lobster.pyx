    ...
    def __init__(self,LOB,messagefile,
        ...
        if aggregate_time_stamp:
            ...
            if add_hawkes_marks:
                ...
                lob=self.aggregated_LOB.merge(mf[['original_idx']],on='original_idx',how='right')
    ...
    def aggregate_LOB_and_messagefile(self,list eventTypes_to_aggregate=[4,5],
        ...
        for j in range(num_iter):
            ...
            if self.separate_13_events:
                mf=separate_pair_events(mf,first_eventType=1,second_eventType=3)
            if self.separate_31_events:
                mf=separate_pair_events(mf,first_eventType=3,second_eventType=1)
            if self.separate_41_events:
                mf=separate_pair_events(mf,first_eventType=4,second_eventType=1)
            if self.separate_34_events:
                mf=separate_pair_events(mf,first_eventType=3,second_eventType=4)
            if self.separate_43_events:
                mf=separate_pair_events(mf,first_eventType=4,second_eventType=3)
...
class ManipulateOrderBook:
    ...
    def encode_sd_hawkes_states(self):
        ...
        lob=self.LOB.copy()
        lob['one_dim_state']=np.array(states_df['one_dim_label'].values,dtype=DTYPEi,copy=True)
    ...
    def create_df_of_volume_proportions(self):
        ...
        try:
            ...
            pass
        volumebook=volumebook.merge(df_state_enc,on='one_dim_state',how='left') 
...
class DataToStore:
    ...
    def store_states(self):
        assert np.all(self.states_df['one_dim_label'].values==self.LOB['one_dim_state'].values)
    ...
    def store_times(self,DTYPEf_t time_origin=0.0):
        ...
        cdef np.ndarray[DTYPEf_t, ndim=1] times =\
              -time_origin + np.array(self.messagefile['time'].values,dtype=DTYPEf)
