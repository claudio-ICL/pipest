...
a="p"
jobid_p=$(qsub -l walltime=02:00:00 -l select=1:ncpus=5:mem=20gb -N "calibr_main-p" -v "$input, a=$a" -W depend=afterok:$jobid_r wrapper_submission.sh)
echo "submission of 'pre-estimate' job with jobid: $jobid_p"
a="c"
jobid_c=$(qsub -l walltime=02:00:00 -l select=1:ncpus=8:mem=30gb -N "calibr_main-c" -J 0-3 -v "$input, a=$a" -W depend=afterok:$jobid_p wrapper_submission.sh)
